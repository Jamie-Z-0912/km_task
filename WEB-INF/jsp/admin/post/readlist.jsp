...
<div id="main-content" class="clearfix">
	<div id="page-content" class="clearfix">
		...
		<div class="row-fluid">
			<div class="row-fluid">
			    <form action="admin/post/readlist" method="get" id="basic_validate" name="basic_validate" class="form-horizontal" novalidate="novalidate">
					...
					<tbody>
						<c:forEach items="${list}" var="log" varStatus="st">
							<tr>
                                ...
                                <td>
                                    <c:forEach items="${objTypes}" var="type" varStatus="st">
                                        <c:if test="${type.type == log.objType}">${type.desc}</c:if>
                                    </c:forEach>
                                </td>
						...
						<ul>
							<c:choose>
								<c:when test="${page gt 1}">
									<li><a
										href="admin/post/readlist?page=${page - 1}&startTime=${startTime}&endTime=${endTime}&uid=${uid}&phone=${phone}&postId=${postId}">&lt;&lt;</a></li>
								</c:when>
								<c:otherwise>
							...
							<c:choose>
								<c:when test="${page gt 1}">
									<li><a
										href="admin/post/readlist?page=1&startTime=${startTime}&endTime=${endTime}&uid=${uid}&phone=${phone}&postId=${postId}">1</a></li>
								</c:when>
								<c:otherwise>
							...
							<c:choose>
								<c:when test="${totalPage ge 2 and page ne 2}">
									<li><a
										href="admin/post/readlist?page=2&startTime=${startTime}&endTime=${endTime}&uid=${uid}&phone=${phone}&postId=${postId}">2</a></li>
								</c:when>
								<c:otherwise>
							...
							<c:choose>
								<c:when test="${totalPage ge 3 and page ne 3}">
									<li class=""><a href="admin/post/readlist?page=3&startTime=${startTime}&endTime=${endTime}&uid=${uid}&phone=${phone}&postId=${postId}">3</a></li>
								</c:when>
								<c:otherwise>
...
$("#submenu_post_read_record").addClass('active');
