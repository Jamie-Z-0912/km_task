        ...
        <div class="control-group">
            ...
            <script type="text/javascript">
                ...
                var option = {
                    ...
                    legend: {
                        data: ['团队数量', '有效团队占比', '团队平均人数', '团队人均收益']
                    },
                    toolbox: {
                        show: true,
                        feature: {
                    ...
                    series: [
                        {
                            ...
                            data: [
                                <c:forEach items="${tjResult}" var="tj" varStatus="st"><c:if test="${st.index != 0}">, </c:if>${tj.teamNum}</c:forEach>
                            ]
                        },
                        {
                            name: '有效团队占比',
                            ...
                            data: [
                                <c:forEach items="${tjResult}" var="tj" varStatus="st"><c:if test="${st.index != 0}">, </c:if>${tj.validTeamRate}</c:forEach>
                            ]
                        },
                        {
                            name: '团队平均人数',
                            ...
                            data: [
                                <c:forEach items="${tjResult}" var="tj" varStatus="st"><c:if test="${st.index != 0}">, </c:if>${tj.averageMemberNum}</c:forEach>
                            ]
                        },
                        {
                            name: '团队人均收益',
                            ...
                            data: [
                                <c:forEach items="${tjResult}" var="tj" varStatus="st"><c:if test="${st.index != 0}">, </c:if>${tj.averageCoin}</c:forEach>
                            ]
                        }
                    ]
                };
...
function quickQuery3(){
    var endtime = ${today_end_time};
    var starttime = endtime - (3*24*60*60*1000);
    window.location.href = "admin/tj/team?start_time=" + starttime;
}
